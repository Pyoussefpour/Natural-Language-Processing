...
class TransformerRunner:
            ...
            if (
                ...
                with torch.no_grad():
                    bleu = self.compute_average_bleu_over_dataset(
                        ...
                        use_greedy_decoding=self.opts.greedy,  # changed
                    )
    ...
    def train_for_epoch(
        ...
        for i, (source, source_lens, targets) in enumerate(tqdm(dataloader, desc="Training")):
            source = source.to(device)
            source_lens = source_lens.to(device)
            targets = targets.to(device)

            model_input, criterion_target = self.train_input_target_split(targets)
            logits = self.model(source, model_input,True)
            logits = logits.view(-1, logits.size(-1))
            loss = criterion(logits, criterion_target)
            loss /= accum_iter
            loss.backward()
    ...
    def translate(self, input_sentence):
        ...
        source_tokens = [self.dataset.source_word2id.get(word,self.dataset.source_unk_id) for word in tokenized_sentence]
        source_tokens = torch.tensor(source_tokens).unsqueeze(0)
        # === ============== === #
        ...
        translated = ' '.join([self.dataset.target_id2word[id.item()] for id in hypotheses[0]])
    ...
    def compute_batch_total_bleu(
        ...
        batch_blue_score = {level: 0.0 for level in n_gram_levels}
        for ref, candid in zip(target_y_ref, target_y_cand):
            mod_ref = [token.item() for token in ref if token.item() not in (sos_idx,eos_idx,pad_idx)]
            mod_candid = [token.item() for token in candid if token.item() not in (sos_idx,eos_idx,pad_idx)]

            for level in n_gram_levels:
                blue_score = bleu_score_func(mod_ref,mod_candid,level)
                batch_blue_score[level] += blue_score
                

        summed_BLEU_score = tuple(batch_blue_score[level] for level in n_gram_levels)
        batch_size = target_y_cand.size(0)

        return summed_BLEU_score, batch_size
